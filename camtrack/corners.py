...
def _build_impl(frame_sequence: pims.FramesSequence,
    ...
    MIN_DISTANCE = 10  # минимальное расстояние в пикселях между уголками, которые ищем (берем больше, чтобы не кучковались)
    QUALITY_LEVEL = 0.003  # качество уголков - чем меньше, тем больше уголков найдём, но хуже качество будет
    BLOCK_SIZE = 21  # размер блока внутри изображения, по которому считаются частные производные для нахождения уголков
    # (так как тут нет поиска уголков с помощью пирамид (я так и не реализовал), то берем побольше
    ...
    lks = dict(winSize=(25, 25),  # оптять же, окно для подсчет частных производных берем побольше
               maxLevel=5,  # количество пирамид, которые использует Лукас-Канада для отслеживания потока
               criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 100, 0.01))  # критерий остановки отслеживания
    # потока: либо cv2.TERM_CRITERIA_EPS (эпсилон - погрешность поиска) станет < 0.01, либо число итераций при
    # отслеживании cv2.TERM_CRITERIA_COUNT стало > 100
