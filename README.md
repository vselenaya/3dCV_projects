...
(после выполнения всех трёх частей) - это полноценный трэкер камеры, который может по
...
2) Проверить эффективность решения конкретного домашего задания можно с помощью
...
среднее качество трекинга по всем тестам (чем ближе к 1 - тем лучше).\
\
Пример запуска:\
...
и заключается трекинг уголков. В какой-то момент уголок может пропасть со следующего кадра - это значит, что на новом кадре уголок отследить не удалось, то есть трек оборвался. Так 
же на новых кадрах могут повляться новые уголки - это значит, что помимо трекинга старых
уголков с предыдущих кадров, мы задетектировали новые уголки на новом кадре.

### Домашнее задание 2 - Трэкер камеры. Часть 1.
...
нам известны характеристики камеры такие, как фокусное расстояние). Положение камеры задаётся матрицей поворота и вектором переноса.
...
усложняет её, ведь нам могут дать очень неудобные начальные кадры (на большм расстоянии
...
[файл3] [файл4] [файл5] [файл6]`, где:
    * `файл1` - истинные положения камеры
    ...
    * `файл3` - видеофайл, для которого трэкаем камеру
...
2) Чтобы немного визуализировать трэкание камеры, можно дополнительно передать
в предыдущие коамнды параметр `--show`. Тогда появится окно, где покадрово будет отрисовываться видео,
а на каждом кадре будут отрисованы синие отрезки - "невязки", отражающие ошибку
трэкинга камеры (чем они длиннее, тем хуже результат трэкинга)

### Домашнее задание 3 - Трэкер камеры. Часть 2.
...
и возвращает эти кадры вместе с найденными положениями. Это называетя
 инициализацией. Далее начинается трэкинг из
предыдущей части. Таким образом, по результату домашнего задания 3 у нас получается
полностью работоспособный трэкер камеры, который только по видео может сначала посчитать уголки,
а затем с их помощью найти положения камеры в каждом кадре видео.

Важно: конечно, по видео невозможно определить масштаб, поэтому весь трэкинг
...
параметрам трэкинга камеры, которые приводили в предыдущей части (наиболее
...
выходе файл с положениями камеры (в видк матрицы поворота и вектора переноса) в каждом кадре.
...
метрикой трэкинга камеры. Оно всегда находится от 0 до 1 и чем ближе к 1, тем лучше
мы произвели трэкинг (тем точнее определенные нами положения камеры). Для сложных тестов
...
хорошим результатм считается `Volume under surface` равный 0.8...0.85. Для остальных
...
полезен, чтобы понимать, в каком кадре мы сильно ошиблись при трэкинге); второй
...
прижётся вручную через `pip install` утанавливать.
...
хотя файлы `corners.py` у нас написан и лежит в той же директории), то чтобы это исправить,
...
происходит очень хорошо - это можно отследить визуально. Да и трэкинг камеры (домашнее
...
400 кадров) считаются не дольше пары минут, сам трэкинг тоже работает довольно быстро. 
...
эта часть может работать минут 20. Весь процесс трэкинга выводится на экран, что
